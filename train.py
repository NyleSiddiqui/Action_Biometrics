...
def train_epoch(epoch, data_loader, model, optimizer, ema_optimizer, criterion, writer, use_cuda, flag, args,
    ...
    if flag:
        ...
        for i, (clips, ss_clips, sa_clips, targets, actions, _) in enumerate(tqdm(data_loader)):
            ...
            if 3 < i < 5:
                ...
                print(f'pred sub: {output_subjects}, GT: {targets}, pred act: {output_actions}, GT: {actions}, features: {features.shape}',
                    flush=True)
    ...
    print('Training Epoch: %d, Loss: %.4f, SL: %.4f, AL: %.4f, SCL: %.4f, ACL: %.4f, OSL: %.4f, OAL: %.4f' % (
    epoch, np.mean(losses), np.mean(supervised_sub_losses), np.mean(supervised_act_losses),
    np.mean(ss_contrastive_losses), np.mean(sa_contrastive_losses), np.mean(ortho_sub_losses),
    np.mean(ortho_act_losses)), flush=True)
...
def val_epoch(cfg, epoch, data_loader, model, writer, use_cuda, args, action_flag=False):
    ...
    for i, (clips, labels, action_targets, keys) in enumerate(tqdm(data_loader)):
        ...
        with torch.no_grad():
            ...
            if args.model_version in ['v1', 'v2', 'v3', 'v3+backbone', 'v4', 'v3_intermediate', 'skeleton2']:
                if args.model_version in ['v3', 'v3+backbone', 'skeleton2']:
                ...
                sacc = torch.sum(output_subjects == labels)
                sub_acc.append(sacc)
            else:
...
def train_model(cfg, run_id, save_dir, use_cuda, args, writer):
    ...
    flag = True if args.model_version in ['v1', 'v2', 'v3', 'v3+backbone', 'v4',
                                          'v3_intermediate', 'skeleton2'] else True if args.contrastive_train else False
    action_flag = False
    ...
    if flag and action_flag:
        ...
        train_dataloader = DataLoader(train_data_gen, batch_size=args.batch_size, shuffle=shuffle,
                                      ...
                                      drop_last=True)  # , collate_fn=multi_default_collate)
        val_dataloader = DataLoader(val_data_gen, batch_size=args.batch_size, shuffle=shuffle,
                                    num_workers=args.num_workers, drop_last=True)  # , collate_fn=multi_val_collate)
    elif flag:
        print('entered2')
        train_dataloader = DataLoader(train_data_gen, batch_size=args.batch_size, shuffle=shuffle,
                                      num_workers=args.num_workers, drop_last=True)#, collate_fn=default_collate)
        val_dataloader = DataLoader(val_data_gen, batch_size=args.batch_size, shuffle=shuffle,
                                    num_workers=args.num_workers, drop_last=True)#, collate_fn=val_collate)
    else:
        print('entered3')
        train_dataloader = DataLoader(train_data_gen, batch_size=args.batch_size, shuffle=shuffle,
                                      num_workers=args.num_workers, drop_last=True,
                                      collate_fn=val_collate)  # r3d charades
        val_dataloader = DataLoader(val_data_gen, batch_size=args.batch_size, shuffle=shuffle,
                                    num_workers=args.num_workers, drop_last=True, collate_fn=val_collate)
    ...
    model = build_model(args.model_version, args.input_dim, args.num_frames, num_subjects, cfg.num_actions,
                        args.patch_size, args.hidden_dim, args.num_heads, args.num_layers)
    ...
    for epoch in range(args.num_epochs):
        # model = train_epoch(epoch, train_dataloader, model, optimizer, ema_optimizer, criterion, writer, use_cuda, flag, args, accumulation_steps=args.steps)  # V3 Charades
