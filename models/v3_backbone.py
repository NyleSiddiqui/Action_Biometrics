...
class VideoTransformer(nn.Module):
    def __init__(self, input_size, num_frames, num_subjects, num_actions, patch_size, hidden_dim, num_heads, num_layers):
        ...
        self.subject_weights = nn.Parameter(torch.ones(num_subjects))
        self.action_weights = nn.Parameter(torch.ones(num_actions))
        
        self.feature_collapse_sub = nn.Linear(self.backbone_output, 1) #TODO
        self.feature_collapse_act = nn.Linear(self.backbone_output, 1)
    ...
    def forward(self, inputs):
        ...
        #features_action = features
        #features_subject = features
        #for i in range(self.num_layers):
            #features_action = self.action_decoder[i](action_tokens, features_action)
        
        #for i in range(self.num_layers):
            #features_subject = self.subject_decoder[i](subject_tokens, features_subject)
        ...
        #output_subjects = self.feature_collapse_sub(features_subject).squeeze()
        #output_actions = self.feature_collapse_act(features_action).squeeze()
